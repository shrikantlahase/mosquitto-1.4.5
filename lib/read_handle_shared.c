...
//	 _mosquitto_log_printf(NULL, MOSQ_LOG_INFO, "Received PINGREQ from %s", mosq->id);
#else
...
//	_mosquitto_log_printf(NULL, MOSQ_LOG_INFO, "Received PINGRESP from %s", mosq->id);
#else
...
#endif
        _mosquitto_log_printf(NULL, MOSQ_LOG_INFO,"read_handle_shared.c:_mosquitto_handle_pubrec: mqtt3_db_message_update() returns(%d)\n",rc);
	if(rc) return rc;
	rc = _mosquitto_send_pubrel(mosq, mid);
        _mosquitto_log_printf(NULL, MOSQ_LOG_INFO,"read_handle_shared.c:_mosquitto_handle_pubrec: _mosquitto_send_pubrel() returns(%d)\n",rc);
	if(rc) return rc;
