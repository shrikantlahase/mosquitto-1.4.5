...
int _mosquitto_packet_alloc(struct _mosquitto_packet *packet)
{
	...
	if(!packet->payload) 
        {
      printf("util_mosq.c: _mosquitto_packet_alloc():_mosquitto_malloc() returns MOSQ_ERR_NOMEM \n");
...
#endif
{
//	_mosquitto_log_printf(NULL, MOSQ_LOG_INFO,"_mosquitto_check_keepalive....START");
	time_t last_msg_out;
	time_t last_msg_in;
	...
	if(mosq->keepalive && mosq->sock != INVALID_SOCKET &&
		...
		if(mosq->state == mosq_cs_connected && mosq->ping_t == 0){
			...
			mosq->listener = NULL;
		_mosquitto_log_printf(NULL, MOSQ_LOG_INFO,"util_mosq.c:mosquitto_check_keepalive:..calling _mosquitto_socket_close\n(mosq_client_state=%d) ",mosq->state);
			_mosquitto_socket_close(db, mosq);
#else
...
//	_mosquitto_log_printf(NULL, MOSQ_LOG_INFO,"_mosquitto_check_keepalive....END");
}
