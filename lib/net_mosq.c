...
int _mosquitto_try_connect(struct mosquitto *mosq, const char *host, uint16_t port, mosq_sock_t *sock, const char *bind_address, bool blocking)
{
    ...
    _mosquitto_log_printf(NULL,MOSQ_LOG_INFO,"net_mosq.c:_mosquitto_try_connect: START..\n");
...
#  endif
	...
	if(mosq->in_packet.remaining_count <= 0){
		...
		if(mosq->in_packet.remaining_length > 0){
                        _mosquitto_log_printf(NULL, MOSQ_LOG_INFO, "net_mosq.c:_mosquitto_packet_read(): mosq->in_packet.remaining_length = %d ",mosq->in_packet.remaining_length);
			mosq->in_packet.payload = _mosquitto_malloc(mosq->in_packet.remaining_length*sizeof(uint8_t));
                        _mosquitto_log_printf(NULL, MOSQ_LOG_INFO,"net_mosq.c:_mosquitto_packet_read: mosquitto_malloc(%ld) returns %s ",mosq->in_packet.remaining_length*sizeof(uint8_t),mosq->in_packet.payload);
...
#  endif
	rc = mqtt3_packet_handle(db, mosq);
        _mosquitto_log_printf(NULL, MOSQ_LOG_INFO,"net_mosq.c:_mosquitto_packet_read: mqtt3_packet_handle()..returns(rc= %d)\n in_packet_command (%X)",rc,(((mosq->in_packet.command)&0xF0)));
#else
