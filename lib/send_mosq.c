...
//       	_mosquitto_log_printf(NULL, MOSQ_LOG_INFO, "Sending PINGREQ to %s", mosq->id); 	//shrikant
#else
	...
	if(mosq) 
		{
			...
			_mosquitto_log_printf(NULL, MOSQ_LOG_INFO, "Sending PUBACK to %s (Mid: %d)", mosq->id, mid);    //shrikant
		}  
#else
	...
	if(mosq)
		{
			 ...
			 _mosquitto_log_printf(NULL, MOSQ_LOG_INFO, "Sending PUBCOMP to %s (Mid: %d)", mosq->id, mid);
		}
#else
...
int _mosquitto_send_command_with_mid(struct mosquitto *mosq, uint8_t command, uint16_t mid, bool dup)
{
	...
	if(!packet)
        {
            _mosquitto_log_printf(NULL, MOSQ_LOG_INFO,"send_mosq.c:_mosquitto_send_command_with_mid(): _mosquitto_calloc() returns error MOSQ_ERR_NOMEM\n");
            return MOSQ_ERR_NOMEM;
        }
