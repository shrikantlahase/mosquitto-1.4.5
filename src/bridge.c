...
int mqtt3_bridge_connect(struct mosquitto_db *db, struct mosquitto *context)
{
	...
	_mosquitto_log_printf(NULL, MOSQ_LOG_NOTICE, "bridge.c:mqtt3_bridge_connect:Connecting bridge %s (%s:%d)", context->bridge->name, context->bridge->addresses[context->bridge->cur_address].address, context->bridge->addresses[context->bridge->cur_address].port);

	_mosquitto_log_printf(NULL, MOSQ_LOG_NOTICE,"bridge.c:DEBUG..1\n");
	...
	_mosquitto_log_printf(NULL, MOSQ_LOG_NOTICE,"bridge.c:  mosquitto_socket_connect()..returns error code= %d.\n",rc);
	if(rc > 0 ){
		if(rc == MOSQ_ERR_TLS){
			_mosquitto_socket_close(db, context);
			return rc; /* Error already printed */
		}else if(rc == MOSQ_ERR_ERRNO){
			_mosquitto_log_printf(NULL, MOSQ_LOG_ERR, "bridge.c:mqtt3_bridge_connect:Error creating bridge: %s.", strerror(errno));
			_mosquitto_log_printf(NULL, MOSQ_LOG_INFO, "bridge.c:mqtt3_bridge_connect:Error creating bridge: %s.", strerror(errno));
		}else if(rc == MOSQ_ERR_EAI){
			_mosquitto_log_printf(NULL, MOSQ_LOG_ERR, "bridge.c:mqtt3_bridge_connect:Error creating bridge: %s.", gai_strerror(errno));
			_mosquitto_log_printf(NULL, MOSQ_LOG_INFO, "bridge.c:mqtt3_bridge_connect:Error creating bridge: %s.", gai_strerror(errno));
		}

		return rc;
	}else {
	
	//	_mosquitto_log_printf(NULL,MOSQ_LOG_NOTICE,"bridge.c:mqtt3_bridge_connect:Connecting Bridge %s (%s:%d) is SUCCESS. ",context->bridge->name, context->bridge->addresses[context->bridge->cur_address].address, context->bridge->addresses[context->bridge->cur_address].port);	 
	}
	...
	_mosquitto_log_printf(NULL, MOSQ_LOG_NOTICE,"bridge.c: mosquitto_send_connect()..returns code =%d.\n",rc);
	...
	if(rc == MOSQ_ERR_SUCCESS){
		...
		if(rc == MOSQ_ERR_TLS){
			...
			_mosquitto_log_printf(NULL, MOSQ_LOG_INFO, "Error creating bridge: %s.", strerror(errno));
		}else if(rc == MOSQ_ERR_EAI){
			_mosquitto_log_printf(NULL, MOSQ_LOG_ERR, "Error creating bridge: %s.", gai_strerror(errno));
			_mosquitto_log_printf(NULL, MOSQ_LOG_INFO, "Error creating bridge: %s.", gai_strerror(errno));
		}
		_mosquitto_socket_close(db, context);
		return rc;
	}
