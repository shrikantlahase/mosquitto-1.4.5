...
int mqtt3_handle_connect(struct mosquitto_db *db, struct mosquitto *context)
{
	...
	static int countt = 0;    //shrikant
	static int count_no_usr_name = 0;  //shrikant
#ifdef WITH_TLS
	...
	if(found_context){
		...
		found_context->state = mosq_cs_disconnecting;
               _mosquitto_log_printf(NULL,MOSQ_LOG_INFO,"read_handle_server.c:mqtt3_handle_connect():calling do_disconnect....5 (context->state=%d)",found_context->state);
	...
	if(db->config->connection_messages == true){
		if(context->is_bridge){
			...
			if(context->username){
				countt=countt+1;
				_mosquitto_log_printf(NULL, MOSQ_LOG_NOTICE, "New client connected from %s as %s (c%d, k%d, u'%s').---->(count = %d )", context->address, client_id, clean_session, context->keepalive, context->username,countt);
			}else{
				count_no_usr_name=count_no_usr_name+1;
				_mosquitto_log_printf(NULL, MOSQ_LOG_NOTICE, "New client connected from %s as %s (c%d, k%d).(NO USERNAME)---->(count = %d )", context->address, client_id, clean_session, context->keepalive,count_no_usr_name);
			}
		}
	}
