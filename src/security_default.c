...
int mosquitto_security_apply_default(struct mosquitto_db *db)
{
	...
	HASH_ITER(hh_id, db->contexts_by_id, context, ctxt_tmp){
       ...
       _mosquitto_log_printf(NULL,MOSQ_LOG_INFO,"security_default.c:mosquitto_security_apply_default():calling do_disconnect....8 (context->state=%d)",context->state);
		...
		if(mosquitto_unpwd_check_default(db, context->username, context->password) != MOSQ_ERR_SUCCESS){
			context->state = mosq_cs_disconnecting;
       _mosquitto_log_printf(NULL,MOSQ_LOG_INFO,"security_default.c:mosquitto_security_apply_default():calling do_disconnect....9 (context->state=%d)",context->state);
			do_disconnect(db, context);
			continue;
		}
